...
export class MatchingService {
  ...
  async status(userId: string) {
    const latest = await this.uploads.findOne({ userId }).sort({ createdAt: -1 }).lean();
    return latest ?? { status: "none" };
  }

  async history(userId: string) {
    return this.uploads.find({ userId }).sort({ createdAt: -1 }).limit(10).lean();
  }

  async versions(userId: string) {
    const uploads = await this.uploads.find({ userId }).sort({ createdAt: -1 }).limit(5).lean();
    const analyses = await this.analyses.find({ userId }).lean();
  ...
  async compare(userId: string, aId: string, bId: string) {
    const [a, b] = await Promise.all([
      this.analyses.findOne({ userId, resumeId: aId }).lean(),
      this.analyses.findOne({ userId, resumeId: bId }).lean(),
    ]);
  ...
  async companyFit(userId: string) {
    const latest = await this.analyses.findOne({ userId }).sort({ createdAt: -1 }).lean();
    return latest?.companyFitScores ?? [];
  }

  async jobFit(userId: string) {
    const latest = await this.analyses.findOne({ userId }).sort({ createdAt: -1 }).lean();
    return latest?.jobFitScores ?? [];
  }
  ...
  async clearChat(userId: string) {
    await this.chats.deleteMany({ userId });
    return { ok: true };
  }

  async ask(userId: string, question: string) {
    const analysis = await this.analyses.findOne({ userId }).sort({ createdAt: -1 }).lean();
